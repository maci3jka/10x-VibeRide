 ...
 * @returns 200 UserStatsResponse | 400/401/403/500 ErrorResponse
 */
export const GET: APIRoute = async ({ locals, url }) => {
  ...
  try {
    const stats = await getUserStats(locals.supabase, parseResult.data);
    return jsonResponse(200, stats);
  } catch (err) {
    const errorMsg = err instanceof Error ? err.message : String(err);
    logger.error({ err, errorMessage: errorMsg }, "Failed to fetch user statistics");

    // Return detailed error in dev mode
    if (isDev) {
      return errorResponse(500, "server_error", `Failed to fetch user statistics: ${errorMsg}`);
    }
    return errorResponse(500, "server_error", "Failed to fetch user statistics");
  }
};
