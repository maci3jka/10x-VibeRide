...
import { buildLink, TooManyPointsError, LinkGenerationError } from "../../../../lib/services/googleMapsLinkService";
  ...
  try {
    ...
    if (itinerary.status !== "completed") {
      logger.warn(
        ...
        "Attempted to generate Google Maps link for non-completed itinerary"
      );
      return new Response(
        JSON.stringify({
